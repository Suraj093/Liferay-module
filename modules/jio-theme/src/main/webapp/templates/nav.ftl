	...
	<ul class="navbar-nav mr-auto">
		<#list nav_items as nav_item>
			...
			<#if nav_item.hasChildren()>
			    <#assign dropdown_button = "dropdown" />
			    <#assign dropdown_toggle_button = "dropdown-toggle" />
			    <#assign data_toggle_button = 'data-toggle="dropdown"' />
			</#if>
			...
			<#else>
				<li class="${nav_item_css_class} nav-item ${dropdown_button}" id="layout_${nav_item.getLayoutId()}" role="presentation">
				<a class="${dropdown_toggle_button} nav-link" ${data_toggle_button} aria-expanded="false" ${nav_item_attr_has_popup} href="${nav_item.getURL()}" ${nav_item.getTarget()}>
					   ${nav_item.getName()}
				</a>
			</#if>
			...
			<#if nav_item.hasChildren()>
			 <#assign
					nav_dropdown_menu = "w-100 mt-0 dropdown-menu-custom"
				/>
			</#if>
			...
			<div class="dropdown-menu ${nav_dropdown_menu}">
				<div class="row mx-0">
					<#list nav_item.getChildren() as nav_child>
	                    ...
	                    <#else>
	                      <a class="dropdown-item custom-dropdown-menu" href="${nav_child.getURL()}">${nav_child.getName()}</a>
	                    </#if>  
                    </#list>
				</div>
			</div>	
